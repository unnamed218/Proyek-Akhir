	...
	<body>
		<form method = "POST" action = "<?php echo site_url('c_transaksi/tambah_penjs');?>">
			...
			<!--   <select name = "no_produk" class = "form-control">
			  ...
			  <input type="text" value="Susu Sapi" class="form-control" readonly>
			  <input type="hidden" value="PR_005" name="no_produk" class="form-control" readonly>
			</div>
...
<?php
		...
		<a <?php if($detail == TRUE){?>href = "<?php echo site_url()."/c_transaksi/selesai_penjs/$id/$total"?>" <?php }else{ ?> disabled="disabled" <?php } ?> class="btn btn-info" role="button">Selesai Belanja</a>
